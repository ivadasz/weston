...
weston_wm_handle_event(int fd, uint32_t mask, void *data)
{
	...
	while (event = ((count || !(mask & WL_EVENT_READABLE)) ? xcb_poll_for_queued_event(wm->conn) : xcb_poll_for_event(wm->conn)), event != NULL) {
...
send_configure(struct weston_surface *surface, int32_t width, int32_t height)
{
	...
	if (window->size_hints.flags & PMinSize) {
		if (window->width < window->size_hints.min_width)
			window->width = window->size_hints.min_width;
		if (window->height < window->size_hints.min_height)
			window->height = window->size_hints.min_height;
		width = window->width + hborder;
		height = window->height + vborder;
	}
	if (window->size_hints.flags & PResizeInc) {
		...
		window->width = ((window->width - bw) / window->size_hints.width_inc) * window->size_hints.width_inc + bw;
		window->height = ((window->height - bh) / window->size_hints.height_inc) * window->size_hints.height_inc + bh;
		if (window->width == 0)
			window->width = 1;
		if (window->height == 0)
			window->height = 1;
